...
const Home = {
  ...
  methods: {
    ...
    async getProducts(profileName) {
      ...
      /*
      let url = 'https://www.instagram.com/lhexatrone/?__a=1';
      alert(url);
  ...
  mounted: function(){
    this.getLikeCookie;
    this.getProducts("valoniafeline")
  .then(pictures => {
    
    for (let item in pictures) {
      console.log(pictures[item].node.edge_media_to_caption.edges[0].node.text);
      
      this.insta.push(pictures[item].node);
      this.products.push({
        id: pictures[item].node.id,
        img: pictures[item].node.thumbnail_src,
        description: pictures[item].node.edge_media_to_caption.edges[0].node.text.substr(0, 50),
        price: 10
      })
     }
     console.log("Pictures:", pictures);
     console.log("insta:", this.insta)
  })
